    ...
    public partial class MainForm : Form
    {
        ...
        private async void BtnStartSearch_Click ( object sender, EventArgs e )
        {
            ...
            try
            {
                ...
                // タイマーを作成して開始
                _uiTimer = new System.Windows.Forms.Timer { Interval = 200 }; // 更新頻度を下げる
                var uiUpdateEvent = new AutoResetEvent( false );

                // 中間ファイルの準備（大量の結果を扱う場合のメモリ削減用）
                string tempFilePath = Path.Combine( Path.GetTempPath(), $"ExcelGrep_{DateTime.Now:yyyyMMdd_HHmmss}.tmp" );
                bool usingTempStorage = false;

                // 結果の最大メモリ保持数
                const int MAX_RESULTS_IN_MEMORY = 10000;
                ...
                {
                    // メモリ使用量モニタリング開始
                    _logService.LogMessage( "メモリ使用量モニタリング開始" );
                    System.Threading.Timer memoryMonitorTimer = new System.Threading.Timer( ( state ) =>
                    {
                        try
                        {
                            long memoryUsage = Process.GetCurrentProcess().WorkingSet64;
                            double memoryUsageMB = memoryUsage / ( 1024.0 * 1024.0 );
                            _logService.LogMessage( $"メモリ使用状況: {memoryUsageMB:F2}MB" );

                            // メモリが8GB以上の場合、結果を一時ファイルに移動
                            if ( memoryUsageMB > 8000 && !usingTempStorage && _searchResults.Count > MAX_RESULTS_IN_MEMORY )
                            {
                                SaveResultsToTempFile( _searchResults, tempFilePath );
                                _searchResults.Clear();
                    ...
                    // 一時ファイルに保存された結果をロード
                    if ( usingTempStorage && File.Exists( tempFilePath ) )
                    {
                        List<SearchResult> tempResults = LoadResultsFromTempFile( tempFilePath );

                        // 一時ファイルの結果を先頭に追加
                        List<SearchResult> combinedResults = new List<SearchResult>();
                        combinedResults.AddRange( tempResults );
                        combinedResults.AddRange( _searchResults );
                        _searchResults = combinedResults;

                        _logService.LogMessage( $"一時ファイルから {tempResults.Count} 件の結果をロードしました。合計 {_searchResults.Count} 件。" );
                    }
                    ...
                    if ( remainingResults.Count > 0 )
                    {
                        ...
                        _searchResults.AddRange( remainingResults );
                        // 結果を再表示
                        DisplaySearchResults( _searchResults );
                    }
                ...
                catch ( OperationCanceledException )
                {
                    ...
                    if ( !isRealTimeDisplay )
                    {
                        ...
                        {
                            List<SearchResult> tempResults = LoadResultsFromTempFile( tempFilePath );
                            _searchResults.InsertRange( 0, tempResults );
                        }
                ...
                catch ( Exception ex )
                {
                    ...
                    if ( !isRealTimeDisplay )
                    {
                        ...
                        if ( usingTempStorage && File.Exists( tempFilePath ) )
                        {
                            ...
                            {
                                _logService.LogMessage( $"一時ファイルのロードエラー: {loadEx.Message}" );
                            }
                        }
            ...
            finally
            {
                ...
                // 未解放のCancellationTokenSourceを解放
                if ( _cancellationTokenSource != null )
                {
                    _cancellationTokenSource.Dispose();
                    _cancellationTokenSource = null;
        ...
        /// 検索結果を一時ファイルに保存
        /// </summary>
        private void SaveResultsToTempFile ( List<SearchResult> results, string filePath )
        {
            _logService.LogMessage( $"検索結果を一時ファイルに保存: {filePath}" );
            ...
            try
            {
                ...
                using ( BinaryWriter writer = new BinaryWriter( fs ) )
                {
                    ...
                    foreach ( var result in results )
                    {
                        writer.Write( result.FilePath ?? string.Empty );
                        writer.Write( result.SheetName ?? string.Empty );
                        writer.Write( result.CellPosition ?? string.Empty );
                        writer.Write( result.CellValue ?? string.Empty );
                    }
                }

                _logService.LogMessage( $"{results.Count}件の結果を一時ファイルに保存しました" );
            }
            catch ( Exception ex )
            {
                _logService.LogMessage( $"一時ファイル保存エラー: {ex.Message}" );
            }
        }
        ...
        /// 一時ファイルから検索結果をロード
        /// </summary>
        private List<SearchResult> LoadResultsFromTempFile ( string filePath )
        {
            _logService.LogMessage( $"一時ファイルから検索結果をロード: {filePath}" );
            ...
            try
            {
                ...
                using ( BinaryReader reader = new BinaryReader( fs ) )
                {
                    ...
                    for ( int i = 0; i < count; i++ )
                    {
                        string filePath2 = reader.ReadString();
                        string sheetName = reader.ReadString();
                        string cellPosition = reader.ReadString();
                        string cellValue = reader.ReadString();
                ...
                _logService.LogMessage( $"{results.Count}件の結果を一時ファイルからロードしました" );
            }
            catch ( Exception ex )
            {
                _logService.LogMessage( $"一時ファイルロードエラー: {ex.Message}" );
            }
